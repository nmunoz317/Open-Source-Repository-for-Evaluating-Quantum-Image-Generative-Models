...
class BrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = nib.load(self.path+self.names[idx]).get_fdata()
        name = self.names[idx]
        mritype=self.names[:(idx+1)].count(name)

        # Normalize the image
        max_=np.max(image[:,:,:,mritype-1])
        min_=np.min(image[:,:,:,mritype-1])
        label=mritype-1
        image=cv2.resize(image[20:210,20:210,self.slice,mritype-1], dsize=(self.height, self.width), interpolation=cv2.INTER_LINEAR)
        normalized_data = ((image - min_) / (max_ - min_))
