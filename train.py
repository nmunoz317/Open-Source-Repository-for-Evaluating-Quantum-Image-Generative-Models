...
from src.models import PatchGAN, QCGAN, PQWGAN, GAN, WGAN, Diffusion, QDenseUndirected, QDenseDirected
        ...
        for i, (x, _) in enumerate(dataloader):
            ...
            if i > counter or counter==0:
                if not saved_initial:
                    ...
                    data_= ((x + 1) / 2).clamp(0, 1)
                    save_image(data_, os.path.join(out_dir, 'real_samples.png'), nrow=5)
                    saved_initial=True

                if model not in ['PQWGAN', 'WGAN']:
                    x = x.reshape(batch_size, image_size[0] * image_size[1])
                ...
                if i % save_interval == 0:
                    ...
                    if model=='PQWGAN' or model=='GAN':
                        test_images= ((test_images + 1) / 2).clamp(0, 1)
                    if model=='WGAN':
                        max_value = torch.max(test_images)
                        min_value = torch.min(test_images)
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        for i, (x, y) in enumerate(dataloader):
            if not saved_initial:
                ...
                save_image(x_, os.path.join(out_dir, 'real_samples.png'), nrow=5)
                saved_initial=True
...
def parse_blood_help():
    ...
    print("-tf, --transformation, type=str: Torchvision transform: CenterCrop, Grayscale, RandomHorizontalFlip, RandomVerticalFlip and ColorJitter (default None).")
...
def parse_PatchGAN():
    ...
    print("-q, --n_data_qubits, type=int: Total number of qubits.")
    ...
    print("-opt, --optimizer, type=str: Optimizer type.")
...
def parse_QDense():
    ...
    print("Model references:")
    print("MIT License")
    print("Copyright (c) 2024 Michael Kölle")
    print("Based on software copyrighted by Michael Kölle, licensed under the MIT License.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("-shape",'--img_shape', help="Image shape", type=int, nargs=2)
    parser.add_argument("-tf",'--transformation', help="Torchvision transform", type=str, default=None)
    ...
    parser.add_argument("-m", "--model", help="model to train", type=str, choices=['PatchGAN', 'QC-GAN', 'PQWGAN', 'GAN', 'WGAN', 'Q-Dense', 'Q-Dense Directed'])
    ...
    parser.add_argument("-q", "--n_data_qubits", help="Total number of qubits", type=int, default=None)
    ...
    if args.dataset=='BloodMNIST':
        dataset=BloodDataset(path=args.path, num_imgs=args.num_imgs, image_shape=args.img_shape, transform=parse_transform(args.transformation))
        dataloader=DataLoader(dataset, args.batch_size, shuffle=True)

    if args.dataset=='BRATS':
        dataset=BrainDataset(path=args.path, slice=args.slice, num_imgs=args.num_imgs, image_shape=args.img_shape, num_classes=args.classes, transform=parse_transform(args.transformation))
        dataloader=DataLoader(dataset, args.batch_size, shuffle=True, num_workers=1)
    
    # List of GAN models
    gans=['PatchGAN', 'QC-GAN', 'PQWGAN', 'GAN', 'WGAN']
    # List of diffusion models
    diffusion=['Q-Dense', 'Q-Dense Directed']
